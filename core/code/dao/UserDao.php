...
class UserDao extends BaseDao {
    ...
    */
    function authenticate($username, $encryptedPassword) {
        ...
        $query = "SELECT * FROM ".User::TABLE_NAME." WHERE username=:username AND password=:password AND enabled=true";
        $stmt = $db->prepare($query);
    ...
    function remove($id)
    {
        $removeAuthors = "DELETE FROM ".Article::AUTHOR_TABLE_NAME." WHERE user_id=:uid";
        $db = getConnection();
        $this->executeModifyStatement($db, $removeAuthors, array("uid" => $id));
